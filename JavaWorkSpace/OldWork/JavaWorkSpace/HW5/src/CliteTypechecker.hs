...
checkStmt ast@(ASTVariableDecl ident ty) = do
  ...
  -- FIXME(done): check for duplicate declaration of the same variable in 
  -- one block
...
checkStmt (ASTCast t a) = do
   (tyA, a') <- typeOf a
   throwTypeErrorIf ((validCast t tyA) /= True) ("Cast from "++(show tyA)++ " to "++(show t) ++ " is not allowed")
   return (ASTCast t a')
   
validCast t u = if t == u then True else 
					if ((t == TyChar) && (u == TyInt)) then True else
						if ((t == TyChar) && (u == TyInt)) then True else
							if ((t == TyChar) && (u == TyFloat)) then True else
								if ((t == TyInt) && (u == TyFloat)) then True else False 
-- FIXME (done) : allow conversions 
...
-- FIXME (done) to deal with Float and Char literals
